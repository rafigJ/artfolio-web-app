...
const PostCard: FC<PostCardProps> = ({ product }) => {
	...
	return (
		<Card
			...
			cover={
				<img
					style={{ maxWidth: 406, maxHeight: 204, objectFit: 'cover', cursor: 'pointer' }}
					alt={product.title}
					src={product.url} onClick={() => navigate(`/posts/${product.id}`)} />
			}
		>
			<Meta
				...
				avatar={
					<Link to='/profile/username'>
						<Avatar src={`https://api.dicebear.com/7.x/miniavs/svg?seed=${product.id}`} />
					</Link>
				}
				description={<Link className='ant-card-meta-description' to='/profile/username'>Автор публикации</Link>}
			/>
